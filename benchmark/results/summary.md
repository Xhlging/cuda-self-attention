...
基于对项目计划文档和源码的深入分析，发现计划中已识别但未实现的关键优化——Tiled kernel 存在 **32-way 共享内存 Bank Conflict**，计划自评估性能损失 20-30%。

### 优化 1：Bank Conflict 修复（核心）

**问题根因：** 原代码共享内存访问 `Q_smem[ti * D + d]`（D=64），CUDA 共享内存有 32 个 bank（每 bank 4 字节）：

```
bank = (ti × 64 + d) % 32 = d % 32    (因为 64 % 32 == 0)
```

同一 warp 内 32 个线程全部命中同一个 bank，每次读写被硬件串行化 32 倍。影响范围：
- Q/K/V 加载到共享内存：32-way **write** conflict
- dot product 循环中 Q_smem 读取：32-way **read** conflict（最热路径）

**修复方案：** 共享内存行 stride 从 `D` 改为 `D_PAD = D + 1 = 65`：

```
bank = (ti × 65 + d) % 32 = (ti + d) % 32    (65 % 32 == 1)
```

每个线程命中不同 bank，stride=1，零冲突。共享内存从 16 KB → 16.25 KB（仍在 48 KB 限制内）。

### 优化 2：Template D 为编译期常量

将 kernel 模板化 `template <int D>`，使 D 成为编译期常量（实际实例化 D=64）：
- Tiled kernel 内层 d 循环 `#pragma unroll` 全量展开（64 次迭代零开销）
- `D_PAD = D + 1` 自然成为 `constexpr`
- Launch wrapper 用 `switch (D)` 分发模板实例，支持未来扩展其他 D 值

### 优化 3：K 和 V 同时加载

将共享内存从 2 区（Q_smem + KV_smem 复用）扩展为 3 区（Q_smem + K_smem + V_smem，共 24,960 字节）：
- 同步点从 4 降为 2（每 tile 节省 ~300 cycles × 2）
- exp 计算与 O 累加融合为单次 jl 遍历，消除中间 scores 回写
- 跳过首 tile 的无用 O_local 缩放

### 优化 4：Naive Kernel 预归一化

在 Phase 3 前将 softmax scores 预归一化 `scores[j] *= inv_sum`，节省 N×(D−1) 次乘法（收益有限，因 naive kernel 本即内存带宽瓶颈）。

### 性能对比

#### Tiled Kernel 绝对性能（us，越小越好）

| N    | 优化前   | 优化后   | 加速比 |
|------|----------|----------|--------|
| 32   | 166.2    | 139.0    | 1.20×  |
| 64   | 308.6    | 306.5    | 1.01×  |
| 128  | 642.4    | 374.6    | 1.71×  |
| 256  | 1,301.6  | 465.4    | 2.80×  |
| 512  | 4,850.3  | 1,351.0  | 3.59×  |
| 1024 | 15,114.2 | 4,684.5  | 3.23×  |
...
-----------------------------------------------------------------
   32 |        316.5 |        139.0 |          243.3 |    2.28x
   64 |        544.1 |        306.5 |          222.1 |    1.78x
  128 |       1030.5 |        374.6 |          159.7 |    2.75x
  256 |       2891.9 |        465.4 |          148.2 |    6.21x
  512 |       8327.5 |       1351.0 |          206.3 |    6.16x
 1024 |      47779.8 |       4684.5 |         1464.3 |   10.20x
...
所有优化后正确性保持不变（2026-06-14 回归测试）：

```
N      | Naive vs Ref  | Tiled vs Ref  | Naive vs Tiled
───────┼───────────────┼───────────────┼───────────────
...
- **Bank Conflict 修复**是最大单一收益项，单独贡献 **1.4×–3.3×** 加速，远超计划预期的 20-30%
- K+V 同时加载在中等等序列长度（N=128–512）额外提供 10–20% 提升；大 N 时因共享内存增大导致 occupancy 从 2→1 block/SM，收益递减
- Tiled kernel 从比 Naive 快 ~2× 提升到 **6–10×**，与 PyTorch math backend 的差距从 7–20× 缩小到 3–4×
- N=64 处性能持平因仅 2 个 tile，同步节省微乎其微且 K+V 版本共享内存更大
- Bank Conflict 是共享内存编程中最隐蔽的性能杀手——代码逻辑完全正确，但访存模式导致硬件串行化，且 nvcc 不会对此产生任何警告
